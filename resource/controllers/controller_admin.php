...
class Controller_Admin
{
    ...
    {
        $edit = new Model_Admin();
        $edit->edit_student($student_id, $birthday, $password, $name, $gender);
    }
    ...
    {
        $edit = new Model_Admin();
        $edit->edit_class($class_id, $class_name);
    }
    ...
    {
        $add = new Model_Admin();
        return $add->add_class($class_name);
    }
    ...
    public function get_list_units()
    {
        ...
        echo json_encode($unit->get_list_units($class_id));
    }
    public function get_dashboard_info()
    {
        ...
        $total = array($admin, $student, $class, $question, $test);
        return $total;
    }
    ...
    public function check_edit_class()
    {
        ...
        if (empty($class_name)) {
            ...
            $this->edit_class($class_id, $class_name);
            $result = json_decode(json_encode($this->get_class_info($class_name)), true);
            $result['status_value'] = "Sửa thành công!";
            $result['status'] = 1;
        }
    ...
    public function check_edit_student()
    {
        ...
        if (empty($name) || empty($gender) || empty($birthday) || empty($student_id) || empty($password)) {
            ...
            $this->edit_student($student_id, $birthday, $password, $name, $gender);
            $result = json_decode(json_encode($this->get_student_info($username)), true);
            $result['status_value'] = "Sửa thành công!";
            $result['status'] = 1;
        }
    ...
    public function check_add_test()
    {
        ...
        if (empty($test_name) || empty($time_to_do) || empty($password)) {
            ...
            if ($add) {
                ...
                $list_unit = $model->get_list_units($class_id);
                foreach ($list_unit as $unit) {
                    $limit = $_POST[$unit->unit];
                    $list_quest = $model->list_quest_of_unit($class_id, $unit->unit, $limit);
