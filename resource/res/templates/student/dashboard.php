                    ...
                    <?php
						if($tests[$i]->status_id != 2)
						{
							...
							if($flag)
								...
								?>
                    <a class="waves-effect waves-light btn modal-trigger full-width" style="margin-bottom: 7px;"
                        href="#do-test-<?=$tests[$i]->test_code?>" id="do_test">Làm Bài</a>
                    <div id="do-test-<?=$tests[$i]->test_code?>" class="modal">
                        <div class="row col l12">
                            <form class="form_test" action="" method="POST" role="form"
                                id="form_submit_test_<?=$tests[$i]->test_code?>">
                                <div class="modal-content">
                                    <h5>Nhập mật khẩu đề: <?=$tests[$i]->test_code?></h5>
                                    <div class="modal-body">
                                        <div class="input-field">
                                            <input type="hidden" value="<?=$tests[$i]->test_code?>" name="test_code"
                                                id="test_code">
                                ...
                                <div class="col l12 s12">
                                    <div class="modal-footer">
                                        <a href="#"
                                            class="waves-effect waves-green btn-flat modal-action modal-close">Trở
                                            Lại</a>
                                        <button type="submit"
                                            class="waves-effect waves-green btn-flat modal-action modal-close">Đồng
                                            Ý</button>
                                    </div>
                                </div>
                            </form>
